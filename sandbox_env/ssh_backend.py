...
class SshSandboxBackend(SandboxBackend):
    ...
    def __init__(self, settings: SandboxSettings, ssh=None):
        ...
        if ssh is None:
            from agent.ssh import SshBackend  # lazy:paramiko 仅真实运行时才引
    ...
    def ensure(self, session_key: str | None = None) -> str:
        ...
        out = self.ssh.exec(f"docker ps -aq --filter name=^/{name}$", timeout=30)
        ...
        if not exists:
            ...
            r = self.ssh.exec(cmd, timeout=120)
            if r.returncode != 0:
    ...
    def exec(self, cmd_str: str, *, session_key: str | None = None,
        ...
        key = session_key or "default"
        name = self.ensure(key)
        full = f"docker exec {name} /bin/bash -lc {shlex.quote(cmd_str)}"
        try:
            raw = self.ssh.exec(full, timeout if timeout is not None else 120)
    ...
    def sync(self, local_dir, session_key: str | None = None) -> None:
        ...
        if hasattr(self.ssh, "sync_to"):
            self.ssh.sync_to(local_dir, rdir)
        else:
