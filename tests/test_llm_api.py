...
def _capture_request(captured):
    ...
    async def fake(client, messages, **kwargs):
...
async def test_chat_injects_docs_into_system(monkeypatch):
    captured = {}
    monkeypatch.setattr(llm_api, "_request", _capture_request(captured))
    await llm_api.chat("问题", docs=["文档1", "文档2"], api_key="test")
...
async def test_chat_appends_docs_to_existing_system(monkeypatch):
    captured = {}
    monkeypatch.setattr(llm_api, "_request", _capture_request(captured))
    await llm_api.chat("问题", system="你是助手", docs=["文档"], api_key="test")
    assert captured["messages"][0]["role"] == "system"
    assert captured["messages"][0]["content"].startswith("你是助手")
    assert "# 可用文档" in captured["messages"][0]["content"]


async def test_chat_docs_truncated_to_budget(monkeypatch):
    captured = {}
    monkeypatch.setattr(llm_api, "_request", _capture_request(captured))
    big = "x" * 500
    await llm_api.chat("问题", docs=[big, big], max_docs_chars=100, api_key="test")
...
async def test_chat_docs_empty_list_noop(monkeypatch):
    captured = {}
    monkeypatch.setattr(llm_api, "_request", _capture_request(captured))
    await llm_api.chat("问题", docs=[], api_key="test")
    assert captured["messages"] == [{"role": "user", "content": "问题"}]


async def test_chat_with_docs_passes_messages_as_is(monkeypatch):
    captured = {}
    monkeypatch.setattr(llm_api, "_request", _capture_request(captured))
    hist = [{"role": "user", "content": "你好"}]
    await llm_api.chat(messages=hist, docs=["文档"], api_key="test")
    msgs = captured["messages"]
...
async def test_chat_returns_content(monkeypatch):
    ...
    assert await llm_api.chat("hi", api_key="test") == "你好"


async def test_chat_request_kwargs(monkeypatch):
    captured = {}
    monkeypatch.setattr(llm_api, "_request", _capture_request(captured))
    await llm_api.chat("hi", system="s", model="m", temperature=0.2, max_tokens=10, api_key="test")
    kw = captured["kwargs"]
    assert kw["model"] == "m"
...
async def test_chat_with_tools_runs_one_round(monkeypatch):
    seen = []
    ...
    def tool_exec(name, args):
        ...
        return {"ok": True}
    result = await llm_api.chat_with_tools("任务", tools=[{}], tool_exec=tool_exec,
...
async def test_chat_with_tools_multi_round(monkeypatch):
    seen = []
    ...
    result = await llm_api.chat_with_tools("任务", tools=[{}], tool_exec=lambda n, a: {"r": n},
                                           api_key="test")
...
async def test_chat_with_tools_exceeds_rounds_raises(monkeypatch):
    async def fake(client, messages, **kwargs):
        return _resp(tool_calls=[_tc("t1")])
    monkeypatch.setattr(llm_api, "_request", fake)

    with pytest.raises(llm_api.ToolLoopError) as ei:
        await llm_api.chat_with_tools("任务", tools=[{}], tool_exec=lambda n, a: {},
                                      api_key="test", max_tool_rounds=3)
    # 已达成的部分轨迹保留在异常上(供上层日志/flag 提取,不因超限丢失)
    assert [t["name"] for t in ei.value.trace] == ["t1", "t1", "t1"]


async def test_tool_exec_exception_recorded_not_fatal(monkeypatch):
    seen = []
    ...
    def tool_exec(name, args):
        raise ValueError("炸了")
    result = await llm_api.chat_with_tools("任务", tools=[{}], tool_exec=tool_exec, api_key="test")
    assert result.content == "继续"
    assert result.trace[0]["result"] == {"error": "ValueError: 炸了"}


async def test_tool_bad_json_arguments(monkeypatch):
    seen = []
...
async def test_chat_with_tools_injects_docs(monkeypatch):
    seen = []
    ...
    await llm_api.chat_with_tools("任务", docs=["技能甲文档"], tools=[{}],
...
async def test_chat_with_tools_no_tools_falls_back(monkeypatch):
    captured = {}
    monkeypatch.setattr(llm_api, "_request", _capture_request(captured))
    result = await llm_api.chat_with_tools("问题", api_key="test")
...
async def test_request_retries_then_succeeds(monkeypatch):
    ...
    resp = await llm_api._request(_fake_client(create), [{"role": "user", "content": "x"}],
                                  model="m", temperature=0.7, max_retries=3, retry_backoff=0)
    assert calls["n"] == 3
    assert resp.choices[0].message.content == "ok"


async def test_request_raises_when_retries_exhausted(monkeypatch):
    calls = {"n": 0}
    ...
    with pytest.raises(llm_api.LLMError):
        await llm_api._request(_fake_client(create), [{"role": "user", "content": "x"}],
                               model="m", temperature=0.7, max_retries=3, retry_backoff=0)
    assert calls["n"] == 3


async def test_request_non_retryable_single_attempt():
    calls = {"n": 0}
