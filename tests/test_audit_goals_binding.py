...
async def test_eval_goals_empty_without_binding(tmp_path):
    ev = _make(tmp_path)
    assert await ev.eval_goals("ctx", [{"id": "g1"}], "[s1] status=PASSED") == []


async def test_eval_goals_nonempty_with_binding(tmp_path):
    ev = _make(tmp_path)
    ev.bindings.goal_evaluator = _deterministic_goal_eval
    res = await ev.eval_goals("ctx", [{"id": "g1"}], "[s1] status=PASSED")
    assert len(res) == 1
    g = res[0]
...
async def test_eval_goals_missing_evidence(tmp_path):
    ev = _make(tmp_path)
    ev.bindings.goal_evaluator = _deterministic_goal_eval
    res = await ev.eval_goals("ctx", [{"id": "g1"}], "[s1] status=FAILED")
    assert len(res) == 1
